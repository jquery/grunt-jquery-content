...
grunt.registerMultiTask( "build-pages", "Process html and markdown files as pages, include @partials and syntax higlight code snippets", function() {
	...
	grunt.utils.async.forEachSeries( files, function( fileName, fileDone ) {
		...
		grunt.utils.async.series([
			function preprocess( continuation ) {
				grunt.helper( "build-pages-preprocess", post, fileName, continuation );
			},
			function process( continuation ) {
...
// Default pre-processor is a no-op, but needs to continue the async flow
grunt.registerHelper( "build-pages-preprocess", function( post, fileName, continuation ) {
	continuation();
});
